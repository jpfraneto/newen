...
export function calculateDayIndex(startingTimestamp, userTimeZone) {
  ...
  try {
    ...
    const dayIndex = Math.floor(timeDifference / (1000 * 60 * 60 * 24));
    // BEfore this return was dayIndex +1 . I deleted that and it seems that I fixed the problem of the day being changed when the UTC time changes, but I need to check that out tomorrow to be sure about it.
    return dayIndex + 1;
  } catch (error) {

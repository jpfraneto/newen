...
export function isValidTimeZone(tz) {
  const timeZonePattern =
    /^(?:A(?:frica|merica|ntarctica|sia)|E(?:tc|urope)|Indian|Pacific)\/[A-Za-z_]+(?:\/[A-Za-z_]+)*$/;
  return timeZonePattern.test(tz);
}
...
export function calculateDayIndex(startingTimestamp, userTimeZone) {
  ...
  try {
    ...
    // Add 3 hours and 33 minutes to the adjustedStartDate
    adjustedStartDate.setHours(adjustedStartDate.getHours() + 3);
    adjustedStartDate.setMinutes(adjustedStartDate.getMinutes() + 33);
...
export async function checkSessionsPercentage(sadhanas, user) {
  const sadhanasWithFilteredSessions = sadhanas.map((sadhana, index) => {
    const filteredSessionsForThisSadhana = user.sadhanaSessions
      .filter((session, index) => {
        return session.sadhanaId === sadhana.id;
      })
      .reduce((uniqueSessions, session) => {
        const sessionIndex = uniqueSessions.findIndex(
          uniqueSession => uniqueSession.sessionIndex === session.sessionIndex
        );
    ...
    return { sadhanaId: sadhana.id, filteredSessionsForThisSadhana };
  });
  return sadhanasWithFilteredSessions;
}
